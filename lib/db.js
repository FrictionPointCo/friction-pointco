...
const CATEGORY_IMAGES_KEY = "fp:category_images";
const LOCAL_CATEGORY_IMAGES_PATH = path.join(process.cwd(), "data", "category-images.local.json");const SEED_PATH = path.join(process.cwd(), "data", "products.seed.json");
...
/**
 * Category cover photos — a simple { EDC: "url", KNIVES: "url", ... }
 * map, separate from products since categories aren't their own
 * database records. Any category not present here just has no custom
 * photo set yet (the homepage falls back to an automatic product photo,
 * then a plain card, in that order).
 */
export async function getCategoryImages() {
  if (hasRedis) {
    const redis = await getRedis();
    const data = await redis.get(CATEGORY_IMAGES_KEY);
    return data || {};
  }
  if (!fs.existsSync(LOCAL_CATEGORY_IMAGES_PATH)) return {};
  try {
    return JSON.parse(fs.readFileSync(LOCAL_CATEGORY_IMAGES_PATH, "utf-8"));
  } catch {
    return {};
  }
}

export async function setCategoryImage(category, url) {
  const current = await getCategoryImages();
  const updated = { ...current, [category]: url };
  if (hasRedis) {
    const redis = await getRedis();
    await redis.set(CATEGORY_IMAGES_KEY, updated);
  } else {
    try {
      fs.writeFileSync(LOCAL_CATEGORY_IMAGES_PATH, JSON.stringify(updated, null, 2));
    } catch {
      throw new Error(
        "Can't save changes yet — the database isn't connected. Add the Redis integration in Vercel (Storage tab), redeploy, then try again."
      );
